...
Hawkears provides a mechanism for excluding certain species from the analysis. The `exclude.txt` file can be found in the  directory (within the data folder) after running `src/00-setup_scratch.sh`. It contains the names of several species that are excluded by default. This file can be edited to exclude more species.
